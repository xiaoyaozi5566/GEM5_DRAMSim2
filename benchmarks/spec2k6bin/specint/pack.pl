...
BEGIN {
# CPU2006 -- @INC is already set properly
#    chdir 't' if -d 't';
#    @INC = '../lib';
    require './test.pm';
}

# CPU2006 -- we don't have everything to test...
#plan tests => 5852;
plan tests => 5182;
...

{
    ...
    my $foo;
    # This just needs to be a file that will definitely be around
    open(BIN, '<./pack.pl') || die "Can't open pack.pl: $!\n";
...

{
 ...
 SKIP: {
    ...
    skip("-- SPEC CPU not testing the IEEE infinity model", 1);
...
foreach my $t (@templates) {
    SKIP: {
        ...
        skip "Quads not tested", 4 if $@;
        $@ = '' if $@ =~ /Invalid type in pack/;
        is( $@, '' );
...
# CPU2006 -- don't do size-dependent tests
...
sub numbers_with_total {
  ...
  # CPU2006 -- 32 is a guaranteed minimum
  $skip_if_longer_than = 32;

  # CPU2006 -- don't test more than 32 bits worth
  #foreach ('', 1, 2, 3, 15, 16, 17, 31, 32, 33, 53, 54, 63, 64, 65) {
  foreach ('', 1, 2, 3, 15, 16, 17, 31, 32) {
    SKIP: {
...
# SPEC CPU -- f removed to avoid FP overflow
   my @codes = qw( x A Z a c C B b H h s v n S i I l V N L p P F d
		   s! S! i! I! l! L! j J);
   my $G;
   if (eval { pack 'q', 1 } ) {
     push @codes, qw(q Q);
   ...
   for my $type (@codes) {
     ...
     for my $count ('', '3', '[11]') {
	   ...
	   for my $move ('', "X$half", "X!$half", 'x1', 'x!8', "x$half") {
	     ...
	     # CPU2006 -- sometimes junk can be different on different platforms
...
SKIP: {
    ...
    # CPU2006 -- don't use long doubles
    #skip "Long doubles not in use", 56 if $@ =~ /Invalid type/;
    skip "Long doubles not tested", 56;
...
# not a b B h H
# CPU2006 -- don't do quads or long doubles
#foreach my $template (qw(A Z c C s S i I l L n N v V q Q j J f d F D u U w)) {
foreach my $template (qw(A Z c C s S i I l L n N v V j J f d F u U w)) {
  SKIP: {
