    ...
    MemoryController(parent,csvOut_,dramsim_log_,outputFilename_, genTrace_, traceFilename_, num_pids_, fixAddr)
{
    ...
    commandQueue = new CommandQueueFA(bankStates,dramsim_log_,tpTurnLength,num_pids_, fixAddr, diffPeriod, p0Period, p1Period); 
...
bool MemoryControllerFA::addTransaction(Transaction *trans)
{
...
void MemoryControllerFA::receiveFromBus(BusPacket *bpacket)
{
    //Change if needed later.
    MemoryController::receiveFromBus(bpacket);
}

void MemoryControllerFA::updateTransactionQueue()
{
...
void MemoryControllerFA::updateReturnTransactions()
{
    MemoryController::updateReturnTransactions();
}

bool MemoryControllerFA::WillAcceptTransaction(uint64_t pid)
{
